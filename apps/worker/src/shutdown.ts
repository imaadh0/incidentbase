...
export function registerShutdownHandlers(options: ShutdownOptions): void {
  ...
  const shutdown = (reason: string, exitCode: number, error?: unknown): Promise<void> => {
    shutdownPromise ??= (async () => {
      if (error === undefined) {
        options.logger.info({ reason }, 'Worker shutdown started');
      } else {
        options.logger.fatal({ err: error, reason }, 'Fatal worker error initiated shutdown');
      }

      const timeout = setTimeout(() => {
        options.logger.fatal('Worker shutdown timed out');
        process.exitCode = 1;
      }, options.timeoutMs);
      timeout.unref();
      ...
      try {
        ...
        options.logger.fatal({ err: shutdownError }, 'Worker shutdown failed');
        process.exitCode = 1;
      } finally {
