...
The Oracle VPS uses `129-154-225-87.nip.io`. Certbot stores its configuration under `infra/nginx/certs` on that host; `fullchain.pem` and `privkey.pem` there are symlinks into Certbot's `live` directory. The `infra/certbot/incidentbase-certbot-renew.timer` checks daily and runs `renew.sh` only when the certificate has fewer than 30 days left. The script stops Nginx for Certbot's standalone HTTP challenge and starts it again even if renewal fails. Install the service and timer in `/etc/systemd/system`, then enable the timer. Keep the certificate and private key out of Git.
